...
parser.add_argument(
    "-d", "--dir", type=str, required=True, help="Path to configuration"
)
...
try:
    ...
    if sub_run.returncode == 0:
        ...
        print(
            f"[ERROR] Tar command failed with exit code {sub_run.returncode}. Details: {sub_run.stderr}",
            file=sys.stderr,
        )
        sys.exit(1)
except subprocess.TimeoutExpired:
    print("[ERROR] Backup process timed out after 60 seconds.", file=sys.stderr)
    sys.exit(1)
